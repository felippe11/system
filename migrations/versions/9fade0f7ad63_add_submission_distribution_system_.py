...
def upgrade():
    ...
    if "distribution_config" not in existing_tables:
        op.create_table(
            'distribution_config',
            ...
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_distribution_config_evento_id_evento'),
            ),
    ...
    if "distribution_log" not in existing_tables:
        op.create_table(
            'distribution_log',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('evento_id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_distribution_log_evento_id_evento'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_distribution_log')),
        )

    if "submission_category" not in existing_tables:
        op.create_table(
            'submission_category',
            sa.Column('id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_submission_category_evento_id_evento'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_submission_category')),
        )

    if "reviewer_profile" not in existing_tables:
        op.create_table(
            'reviewer_profile',
            sa.Column('id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_reviewer_profile_evento_id_evento'),
            ),
            sa.ForeignKeyConstraint(
                ['usuario_id'],
                ['usuario.id'],
                name=op.f('fk_reviewer_profile_usuario_id_usuario'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_reviewer_profile')),
            sa.UniqueConstraint(
                'usuario_id',
                name=op.f('uq_reviewer_profile_usuario_id'),
            ),
        )

    if "spreadsheet_mapping" not in existing_tables:
        op.create_table(
            'spreadsheet_mapping',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('evento_id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['created_by'],
                ['usuario.id'],
                name=op.f('fk_spreadsheet_mapping_created_by_usuario'),
            ),
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_spreadsheet_mapping_evento_id_evento'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_spreadsheet_mapping')),
        )

    if "imported_submission" not in existing_tables:
        op.create_table(
            'imported_submission',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('evento_id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['evento_id'],
                ['evento.id'],
                name=op.f('fk_imported_submission_evento_id_evento'),
            ),
            sa.ForeignKeyConstraint(
                ['submission_id'],
                ['submission.id'],
                name=op.f('fk_imported_submission_submission_id_submission'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_imported_submission')),
        )

    if "reviewer_preference" not in existing_tables:
        op.create_table(
            'reviewer_preference',
            sa.Column('id', sa.Integer(), nullable=False),
            ...
            sa.ForeignKeyConstraint(
                ['category_id'],
                ['submission_category.id'],
                name=op.f('fk_reviewer_preference_category_id_submission_category'),
            ),
            sa.ForeignKeyConstraint(
                ['reviewer_profile_id'],
                ['reviewer_profile.id'],
                name=op.f('fk_reviewer_preference_reviewer_profile_id_reviewer_profile'),
            ),
            sa.PrimaryKeyConstraint('id', name=op.f('pk_reviewer_preference')),
            sa.UniqueConstraint(
                'reviewer_profile_id',
                'category_id',
                name='unique_reviewer_category_preference',
            ),
        )
    with op.batch_alter_table('assignment', schema=None) as batch_op:
        ...
        existing_indexes = {idx['name'] for idx in inspector.get_indexes("assignment")}
        if 'idx_assignment_reviewer' in existing_indexes:
    ...
    with op.batch_alter_table('review', schema=None) as batch_op:
        ...
        if 'idx_review_submission' in existing_indexes:
    ...
    with op.batch_alter_table('submission', schema=None) as batch_op:
        ...
        existing_indexes = {idx['name'] for idx in inspector.get_indexes("submission")}
        existing_uniques = {uc['name'] for uc in inspector.get_unique_constraints("submission")}
        ...
        batch_op.alter_column('evento_id',
               existing_type=sa.INTEGER(),
               nullable=True)
        if 'idx_submission_author_evento' in existing_indexes:
            batch_op.drop_index('idx_submission_author_evento')
        if 'idx_submission_status' in existing_indexes:
            batch_op.drop_index('idx_submission_status')
        if 'uq_submission_author_evento' in existing_uniques:
    ...
    with op.batch_alter_table('variavel_dinamica', schema=None) as batch_op:
        ...
        batch_op.alter_column('tipo',
               ...
               nullable=False)
        if "ativo" in existing_cols:
            batch_op.drop_column('ativo')
        if "opcoes" in existing_cols:
...
def downgrade():
    ...
    with op.batch_alter_table('variavel_dinamica', schema=None) as batch_op:
        ...
        if "ativa" in existing_cols:
            batch_op.drop_column('ativa')
        if "formato" in existing_cols:
            batch_op.drop_column('formato')
        if "obrigatoria" in existing_cols:
    ...
    with op.batch_alter_table('submission', schema=None) as batch_op:
        ...
        existing_indexes = {idx['name'] for idx in inspector.get_indexes("submission")}
        existing_uniques = {uc['name'] for uc in inspector.get_unique_constraints("submission")}
        if 'uq_submission_author_evento' not in existing_uniques:
            batch_op.create_unique_constraint('uq_submission_author_evento', ['author_id', 'evento_id'])
        if 'idx_submission_status' not in existing_indexes:
            batch_op.create_index('idx_submission_status', ['status'], unique=False)
        if 'idx_submission_author_evento' not in existing_indexes:
    ...
    with op.batch_alter_table('review', schema=None) as batch_op:
        ...
        if 'idx_review_submission' not in existing_indexes:
    ...
    with op.batch_alter_table('assignment', schema=None) as batch_op:
        ...
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("assignment")}
        existing_indexes = {idx['name'] for idx in inspector.get_indexes("assignment")}
        if batch_op.f('fk_assignment_submission_id_submission') in existing_fks:
        ...
        if 'idx_assignment_reviewer' not in existing_indexes:
            batch_op.create_index('idx_assignment_reviewer', ['reviewer_id'], unique=False)

    if 'reviewer_preference' in existing_tables:
        op.drop_table('reviewer_preference')
    if 'imported_submission' in existing_tables:
        op.drop_table('imported_submission')
    if 'spreadsheet_mapping' in existing_tables:
        op.drop_table('spreadsheet_mapping')
    if 'reviewer_profile' in existing_tables:
        op.drop_table('reviewer_profile')
    if 'submission_category' in existing_tables:
        op.drop_table('submission_category')
    if 'distribution_log' in existing_tables:
        op.drop_table('distribution_log')
    if 'distribution_config' in existing_tables:
