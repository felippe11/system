...
Revision ID: 997aabdf3c1d
Revises: 0e3ffde07361
Create Date: 2025-09-22 17:40:18.785771
"""
...
revision = "997aabdf3c1d"
down_revision = "0e3ffde07361"
branch_labels = None
depends_on = None

# ENUM objects (não criam TYPE automaticamente)
tipo_relatorio_enum = postgresql.ENUM(
    "ATIVIDADE", "MENSAL", "TRIMESTRAL", "ANUAL", name="tiporelatorio", create_type=False
)
tipo_campo_enum = postgresql.ENUM(
    "TEXTO", "NUMERO", "DATA", "SELECAO", "MULTIPLA_ESCOLHA", "ARQUIVO", "TEXTO_LONGO",
    name="tipocampo", create_type=False
)
...
def upgrade():
    ...
    if not has_table("relatorio_formador_legacy"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["configuracao_id"],
                ["configuracao_relatorio_formador.id"],
    ...
    # ---------------------------------------------------
    # monitor_cadastro_link: add usage_count / drop used
    # ---------------------------------------------------
    if has_table("monitor_cadastro_link"):
        cols_used = has_column("monitor_cadastro_link", "used")
        cols_usage = has_column("monitor_cadastro_link", "usage_count")

        if not cols_usage or cols_used:
    ...
    # ---------------------------------------------------
    # relatorio_formador: alterações condicionais
    # ---------------------------------------------------
    if has_table("relatorio_formador"):
        ...
        with op.batch_alter_table("relatorio_formador", schema=None) as batch_op:
            ...
            # alterar tipos (tolerante a idempotência)
            try:
            ...
            # FKs novas -> criar somente se NÃO existirem
            ...
            # remover colunas antigas se ainda existirem
...
def downgrade():
    # rollback de relatorio_formador
    if has_table("relatorio_formador"):
        with op.batch_alter_table("relatorio_formador", schema=None) as batch_op:
            if not has_column("relatorio_formador", "dados_relatorio"):
                batch_op.add_column(
                    sa.Column("dados_relatorio", postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=False)
                )
            ...
            if not has_fk("relatorio_formador", "fk_relatorio_formador_configuracao_id_configuracao_rela_e433"):
                try:
                    batch_op.create_foreign_key(
                        "fk_relatorio_formador_configuracao_id_configuracao_rela_e433",
                        "configuracao_relatorio_formador",
                        ["configuracao_id"],
                        ["id"],
                    )
                except Exception:
            ...
            if not has_fk("relatorio_formador", "fk_relatorio_formador_atividade_id_oficina"):
                try:
                    batch_op.create_foreign_key(
                        "fk_relatorio_formador_atividade_id_oficina", "oficina", ["atividade_id"], ["id"]
                    )
                except Exception:
                    pass
    ...
    # Não removemos os TIPOs ENUM no downgrade para evitar efeitos colaterais em outras migrações.
