...
def _ensure_enums():
    ...
    op.execute(
        ...
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipopergunta') THEN
                CREATE TYPE tipopergunta AS ENUM ('TEXTO_LIVRE','MULTIPLA_ESCOLHA','ESCALA_NUMERICA','SIM_NAO');
            ELSE
                ALTER TYPE tipopergunta ADD VALUE IF NOT EXISTS 'TEXTO_LIVRE';
                ALTER TYPE tipopergunta ADD VALUE IF NOT EXISTS 'MULTIPLA_ESCOLHA';
                ALTER TYPE tipopergunta ADD VALUE IF NOT EXISTS 'ESCALA_NUMERICA';
                ALTER TYPE tipopergunta ADD VALUE IF NOT EXISTS 'SIM_NAO';
            END IF;
        END
        $$;
        """
...
def upgrade():
    ...
    sa.Column('tipo', postgresql.ENUM('ATIVIDADE', 'MENSAL', 'TRIMESTRAL', 'ANUAL', name='tiporelatorio', create_type=False), nullable=False),
    ...
    sa.Column('tipo', postgresql.ENUM('TEXTO', 'NUMERO', 'DATA', 'SELECAO', 'MULTIPLA_ESCOLHA', 'ARQUIVO', 'TEXTO_LONGO', name='tipocampo', create_type=False), nullable=False),
    sa.Column('obrigatorio', sa.Boolean(), nullable=True),
    ...
    sa.Column('titulo', sa.String(length=255), nullable=False),
    sa.Column('descricao', sa.Text(), nullable=True),
    sa.Column('tipo', postgresql.ENUM('TEXTO_LIVRE', 'MULTIPLA_ESCOLHA', 'ESCALA_NUMERICA', 'SIM_NAO', name='tipopergunta', create_type=False), nullable=False),
    sa.Column('opcoes', sa.Text(), nullable=True),
    ...
    with op.batch_alter_table('checkin', schema=None) as batch_op:
        existing_cols = {col['name'] for col in inspector.get_columns("checkin")}
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("checkin")}
        if "atividade_multipla_id" not in existing_cols:
            batch_op.add_column(sa.Column('atividade_multipla_id', sa.Integer(), nullable=True))
        if "data_atividade_id" not in existing_cols:
            batch_op.add_column(sa.Column('data_atividade_id', sa.Integer(), nullable=True))
        if "turno" not in existing_cols:
    ...
    with op.batch_alter_table('material', schema=None) as batch_op:
        ...
        if "fornecedor" not in existing_cols:
    ...
    with op.batch_alter_table('monitor_cadastro_link', schema=None) as batch_op:
        existing_cols = {col['name'] for col in inspector.get_columns("monitor_cadastro_link")}
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("monitor_cadastro_link")}
        batch_op.add_column(
            sa.Column(
                'usage_count',
                sa.Integer(),
    ...
    with op.batch_alter_table('oficina', schema=None) as batch_op:
        ...
        if "formador_id" not in existing_cols:
    ...
    with op.batch_alter_table('relatorio_formador', schema=None) as batch_op:
        existing_cols = {col['name'] for col in inspector.get_columns("relatorio_formador")}
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("relatorio_formador")}
        if "oficina_id" not in existing_cols:
            batch_op.add_column(sa.Column('oficina_id', sa.Integer(), nullable=True))
        if "status" not in existing_cols:
            batch_op.add_column(sa.Column('status', sa.String(length=20), nullable=True))
        if "observacoes_cliente" not in existing_cols:
