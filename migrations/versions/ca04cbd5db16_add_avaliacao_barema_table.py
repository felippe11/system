"""add_avaliacao_barema_table

Revision ID: ca04cbd5db16
Revises: 0e3ffde07361
Create Date: 2025-09-22 14:42:22.940119
...
def upgrade():
    ...
    with op.batch_alter_table('evento', schema=None) as batch_op:
        existing_cols = {col['name'] for col in inspector.get_columns("evento")}
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("evento")}
...
def downgrade():
    ...
    with op.batch_alter_table('evento', schema=None) as batch_op:
        existing_cols = {col['name'] for col in inspector.get_columns("evento")}
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys("evento")}
        if "submissao_aberta" not in existing_cols:
            batch_op.add_column(sa.Column('submissao_aberta', sa.BOOLEAN(), autoincrement=False, nullable=True))
