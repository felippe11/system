...
def criar_eventos_para_cliente6():
    """Cria eventos para o cliente ID 6 (andre7@teste.com)."""
    
    app = create_app()
    with app.app_context():
        # Verificar se o cliente ID 6 existe
        cliente = Cliente.query.filter_by(id=6).first()
        if not cliente:
            print("Erro: Cliente ID 6 não encontrado!")
            return
        
        print(f"Cliente encontrado: ID {cliente.id}, Email: {cliente.email}")
        
        # Verificar eventos existentes do cliente
        eventos_existentes = Evento.query.filter_by(cliente_id=6).all()
        print(f"Eventos existentes do cliente: {len(eventos_existentes)}")
        
        # Criar eventos de teste para o cliente ID 6
        eventos_teste = [
            {
                "nome": "Congresso de Tecnologia 2025 - Cliente 6",
            ...
            {
                "nome": "Workshop de Python - Cliente 6",
            ...
            {
                "nome": "Seminário de IA - Cliente 6",
        ...
        print("\nCriando eventos para o cliente ID 6...")
        ...
        try:
            ...
            eventos_apos = Evento.query.filter_by(cliente_id=6).all()
            print(f"\nTotal de eventos do cliente ID 6 após criação: {len(eventos_apos)}")
